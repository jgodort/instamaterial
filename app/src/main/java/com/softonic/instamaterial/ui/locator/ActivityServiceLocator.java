...
public class ActivityServiceLocator
  ...
  @Override public GetCommentItems getCommentItems() {
    if (getCommentItems == null) {
      getCommentItems =
          new GetCommentItems(useCaseExecutor(), getPhotoComments(), getCommentItem());
    }
  ...
  @Override public SignIn signIn(FragmentActivity fragmentActivity) {
    if (signIn == null) {
      signIn = new SignIn(useCaseExecutor(), updateUser(), fragmentActivity);
    }
    return signIn;
  }

  @Override public SignOut signOut(FragmentActivity fragmentActivity) {
    if (signOut == null) {
      signOut = new SignOut(useCaseExecutor(), fragmentActivity);
    }
    return signOut;
  }
